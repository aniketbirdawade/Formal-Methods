...
def tokenize_statements(source: str) -> list[str]:
    ...
    for line in lines:
        ...
        # Keep if / while / for conditions as pseudo-statements
...
def extract_expressions(stmt: str) -> set[str]:
    ...
    # Find all  operand OP operand  patterns
    pattern = re.compile(
...
def defined_var(stmt: str) -> str | None:
    """Return the variable defined by an assignment, or None."""
    if stmt.startswith('COND:'):
        return None
...
def analyse(filepath: str):
    ...
    iters = solve_available_expressions(blocks, entry_id, exit_id, all_exprs)

    print_results(blocks, entry_id, exit_id, all_exprs, iters, filepath)
